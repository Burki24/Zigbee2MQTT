...
| **4**      | **Geräteinformationen**         | Hier wird der Link zum Gerät in der Z2M Doku angezeigt und das entsprechende Bild von dem Gerät. Die Bilder werden von Z2M bereit gestellt und können teilweise abweichen. Sie werden modellbezogen unter `user/IPSZigbee2MQTT/icons` zwischengespeichert und erst beim Öffnen der Konfiguration geladen, damit sie den globalen Symcon-Snapshot nicht unnötig vergrößern. Der Download ist zeitlich und in der Dateigröße begrenzt; ungültige oder ungewöhnlich große PNG-Dateien werden nicht übernommen.                                                                                                                                                                                                                              |
| **5**      | **Geräteinformationen aktualisieren** | Über diesen Button können alle Informationen zu einem Gerät aus Z2M erneut abgerufen werden. Dies ist manchmal notwendig, wenn das Gerät bezüglich der betreffenden Daten (Exposes) aus Z2M ein Update erhalten hat, z. B. neue Effekte oder zusätzliche Datenpunkte. Beim Anlegen der Instanz wird dies automatisch durchgeführt. Nach der manuellen Aktualisierung zeigt die Konfiguration eine verständliche Erfolgs- oder Fehlermeldung. Ist in einer bestehenden Instanz noch keine IEEE-Adresse konfiguriert, wird eine erkannte Adresse ausschließlich in das Formularfeld eingetragen. Prüfen Sie die Adresse und speichern Sie sie anschließend regulär mit **Übernehmen**. |
| **6**      | **Erweiterte Geräteeinstellungen** | Enthält die regulären, aber seltener benötigten Bereiche für Visualisierung, Gerätewartung, Geräteoptionen, Binding und Reporting sowie Variablenverwaltung. Es werden nur Bereiche angezeigt, die für das Gerät fachlich verfügbar sind.                                                                                                                                                                                                                                                                                                                                                                                                             |
| **7**      | **Expertenwerkzeuge**           | Enthält besonders wirkungsvolle oder selten benötigte Funktionen wie erweiterte Geräteentfernung, Variablen-Wartung, Debug-Export, Änderung der IEEE-Adresse und fehlende Übersetzungen. Das Testcenter befindet sich als eigener Bereich auf der obersten Formularebene.                                                                                                                                                                                                                                                                                                                                                                              |
| **8**      | **Dokumentation**               | Direkter Zugriff auf die Dokumentation der Instanz.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **9**      | **Gateway konfigurieren**       | Unter diesem Punkt kann der verbundene MQTT-Splitter (Client oder Server) aufgerufen werden.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| **10**     | **Gateway ändern**              | Dient zur Auswahl des von der Instanz genutzten MQTT-Splitters (Client oder Server). Wird beim Anlegen von Geräten über den [Konfigurator](../Configurator/README.md) automatisch gesetzt und kann auch über diesen korrigiert werden.                                                                                                                                                                                                                                                                                                                                                                                                                |
| **11**     | **InstanzID kopieren**          | Kopiert die Instanz-ID in die Zwischenablage.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
Die Konfiguration priorisiert die tägliche Bedienung: **Visualisierung** bleibt als eigener Bereich sichtbar. Seltener benötigte, aber regulär unterstützte Funktionen wie **Gerätewartung**, **Geräteoptionen**, **Variablen** sowie **Binding und Reporting** befinden sich gesammelt unter **Erweiterte Geräteeinstellungen**. Erweiterte Geräteentfernung, Variablen-Wartung, Debug-Export, IEEE-Adressänderung und fehlende Übersetzungen sind unter **Expertenwerkzeuge** zusammengefasst. Das Testcenter ist davon getrennt direkt auf der obersten Formularebene erreichbar.

1. **Geräteinformationen prüfen oder neu abrufen**
   Kontrollieren Sie, ob Gerätebild, Modell-Link, IEEE-Adresse und MQTT-Topic passen. Wenn Zigbee2MQTT neue oder geänderte Exposes liefert, können die Informationen über **Geräteinformationen aktualisieren** neu geladen werden. Eine erfolgreiche Aktualisierung wird im Formular bestätigt. Eine dabei erkannte, bislang nicht konfigurierte IEEE-Adresse wird nur als noch nicht gespeicherter Formularwert eingetragen. Prüfen Sie die Adresse und klicken Sie anschließend regulär auf **Übernehmen**, um sie zu speichern.
